{% if pillar.get('nightly') == 'True' %}
{% set nightly = salt['cmd.run']('\'python -c "import datetime; today = datetime.date.today(); print today;"\'') %}
{% endif %}
...
OutFile "Salt-Minion-{{ salt['cmd.run']('python c:\\repos\\salt\\salt\\version.py') }}-{{ grains.get('cpuarch', 'amd64') }}-Setup{{ salt['pillar.get']('nightly') }}.exe"
InstallDir "c:\salt"
